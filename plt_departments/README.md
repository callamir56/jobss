...
| `/k9track <id>` | ردیابی پلیر با آیدی توسط سگ (حداکثر ۱۰۰ متر) |
| `/k9attack <id>` | حمله سگ به پلیر با آیدی (حداکثر ۱۰۰ متر) |
| `/k9menu` (یا F7) | منوی کنترل سگ پلیس |

## 🐕 سیستم سگ پلیس (K9)

سگ رو از نود K9 دپارتمان (توی `/managedepts`) اسپان کن. بعد **با ALT روی خود سگ (ox_target)** این گزینه‌ها هست:

- 🎯 **حمله / ردیابی با آیدی:** یه باکس باز میشه، **آیدی پلیر** رو وارد می‌کنی و دکمه **ATTACK** یا **TRACK** رو میزنی. اگه پلیر وجود داشته باشه و تا ۱۰۰ متر نزدیک باشه نوتیف میاد و سگ **پیاده** میره سمتش؛ وقتی رسید حمله می‌کنه (یا برای ردیابی می‌شینه). اگه پلیر نباشه یا دور باشه، نوتیف خطا میاد
- 💥 **حمله به نزدیک‌ترین:** به نزدیک‌ترین پلیر (زیر ۱۰ متر) حمله می‌کنه
- 🚗 **سوار/پیاده ماشین:** سگ **فوری و بدون انیمیشنِ باز کردن در** سوار ماشینی که خودت توشی میشه (اگه پیاده باشی، نزدیک‌ترین ماشین)؛ دوباره بزنی پیاده میشه
- 🪑 **نشستن:** سگ می‌شینه
- 📍 **ردیابی نزدیک‌ترین فرد:** نزدیک‌ترین پلیر رو پیدا می‌کنه و پیاده میره سمتش
- 📋 **باز کردن منوی کامل سگ** (همون منوی F7)

**دستورهای معادل (بدون منو):**
```
/k9track 2     ← ردیابی: پیاده میره سمت آیدی ۲ و وقتی رسید می‌شینه
/k9attack 2    ← حمله: میره سمت آیدی ۲ و وقتی رسید حمله می‌کنه
```
هر دو حداکثر ۱۰۰ متر فاصله رو قبول می‌کنن؛ اگه پلیر توی استریم نباشه یا دورتر باشه، نوتیف خطا میاد.

بقیه دستورها (دنبال کردن، ماندن، پارس، خوابیدن، بو کشیدن، سرچ ماشین) توی منوی **F7** هستن.
